...
## Verification
- Revision checked: `23cc5fd` based on `09f573e`.
- Commands and observed outcomes: `uv run pytest -q` passed 85 tests including all four .NET/Python export modes; Ruff, mypy, schema freshness, sample validation, offline evaluation, mocked browser flow and .NET Release build passed. The 17-case offline quality gate passed; per-class counts are in the generated report, not a live model benchmark.
