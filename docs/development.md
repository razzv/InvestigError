...
`api.py` serves static assets and packaged examples. `POST /api/validate`, `/api/analyze`, and `/api/explain` accept a multipart `file` field with a `.json` or `.jsonl` filename. Validation returns sanitized input, input limitations and the exact bounded provider context; an oversized context returns `preview_error`. Analysis returns JSON report and Markdown. Explain requires `allow_cloud=true`; it revalidates and recomputes the bundle and returns the rules report even when AI is unavailable. `GET /api/examples` lists allowlisted examples; `GET /api/examples/{id}` loads one. The service binds to loopback by default and rejects non-loopback `serve` hosts. Public hosting needs separate authentication and security design.
